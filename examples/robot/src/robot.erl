...
start(_Type, _Args) ->
    ...
    lager:info("robot Supervisor started"),
    ...
    timer:sleep(15000),
    lager:info("partisan_config:init() has been executed"),
    grisp_led:color(1, green),
    {ok, PartisanSup} = partisan_sup:start_link(),
    timer:sleep(15000),
    lager:info("partisan_sup started with PID ~p", [PartisanSup]),
    grisp_led:color(2, green),
    {ok, LaspSup} = lasp_sup:start_link(),
    timer:sleep(15000),
    lager:info("lasp_sup started with PID ~p", [LaspSup]),
