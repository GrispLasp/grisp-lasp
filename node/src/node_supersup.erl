...
%% API
-export([set_config/0, start_link/0, start_link/1, start_node/0, start_partisan/0, start_lasp/0, stop_child/1, delete_child/1, stop_and_delete_child/1]).
...
start_partisan() ->
  partisan_config:set(partisan_peer_service_manager, partisan_hyparview_peer_service_manager),
  {ok, PartisanSup} = supervisor:start_child(node, ?PARTISAN_SPEC),
  {ok, PartisanSup}.

start_lasp() ->
  {ok, LaspSup} = supervisor:start_child(node, ?LASP_SPEC),
  {ok, LaspSup}.
